...
async def inference(
    ...
    try:
        ...
        if audio_file is not None:
            ...
            if audio_id in AUDIO_FEATURE_CACHE:
                ...
                try:
                    ...
                    wav, sr = load_audio_robust(content, audio_file.filename)
        ...
        def run_stage(s_idx, current_prompt, prev_context=""):
            prompts = {
                1: f"请分析这段音频并进行【意图识别】。请识别音频类型（如语音、器乐、环境音等）并简述其表达的核心意图。另外，请给该任务的理解难度打个分（0-100），格式为【难度评分：数字】。用户输入：{current_prompt}",
                2: f"结合刚才的分析：{prev_context}\n请开始【知识检索】。请提取音频中相关的知识点（如乐器、曲风或背景信息等），尽量用结构化的方式表达。请以【知识检索】开头输出。",
                3: f"基于已有的知识：{prev_context}\n请进行【逻辑推演】。分析音频的节奏、旋律或情感是如何推导出上述结论的。请以【逻辑推演】开头输出。",
                4: f"综合以上推理过程：{prev_context}\n请给出【结论总结】。直接回答用户的核心问题即可。请以【最终结论】开头输出。"
            }
            ...
            # 过滤常见的社交辞令/结束语 (作为后置保障，这样 Prompt 就不需要写得太死板)
            filler_patterns = [
        ...
        if stage > 0:
            ...
            # 性能核心优化：使用 One-shot Chain of Thought 模式，显著减少推理轮次与资源占用
            combined_prompt = f"""请对用户音频进行深度分析，并严格按照以下格式一次性输出：
【意图识别】
分析音频类型及意图。
【知识检索】
提取乐器、曲风等背景知识。
...
用户输入：{prompt}"""
            ...
            full_res = generate_pipeline_step(combined_prompt, wav, sr)
